...
namespace TCGShopNewCardsModPreloader.Handlers
{
    ...
    {
        public static List<Monster> monsterMax = new List<Monster>();

        public static void ChangeMaxValues(TypeDefinition enumType, List<Monster> maxValues)
        {
            ...
            ModifyEnumValue(enumType, "MAX", maxValues[0].max);
            foreach (var monster in maxValues.Skip(1))
            {
                ModifyEnumValue(enumType, "MAX_" + monster.name.ToUpper(), monster.max);
            }
        }
